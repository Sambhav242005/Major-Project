...
export default function DocumentsPage() {
  ...
  // Live progress tracking for processing documents
  ...
  useEffect(() => {
    ...
    const refresh = () => { if (!cancelled) fetchDocuments(); };
    ...
    const openStream = (token: string) => {
      evtSource = new EventSource(
        withProject(`/documents/${activeWatchId}/stream?token=${encodeURIComponent(token)}`, activeProjectId)
      );
      ...
      evtSource.onmessage = (event) => {
        try {
          ...
          if (data.stage === "complete" || data.status === "failed") finishWatch();
        } catch {}
      };
        ...
        {loading ? (
          ...
          <Card className="bg-app-card border border-app-border">
            <CardContent className="py-12 text-center">
              ...
              <p className="text-slate-500 text-sm">Upload your first PDF to start building the knowledge base</p>
            </CardContent>
          </Card>
        ) : (
          <Card className="bg-app-card border border-app-border">
                  ...
                  {documents.map((doc) => (
                    <TableRow key={doc.id} className="border-app-border">
                      <TableCell>
                        ...
                        {doc.errorMessage && (
                          <p className="text-red-600 dark:text-red-400 text-xs mt-0.5 truncate max-w-xs" title={doc.errorMessage}>
                            {doc.errorMessage.length > 80 ? doc.errorMessage.slice(0, 80) + "..." : doc.errorMessage}
                          </p>
                        )}
                      </TableCell>
                      ...
                      <TableCell className="text-sm text-app-muted font-mono">{doc.pageCount ?? "—"}</TableCell>
