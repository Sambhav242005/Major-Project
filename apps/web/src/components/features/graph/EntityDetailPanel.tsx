...
export function EntityDetailPanel({ entity, chunks }: EntityDetailPanelProps) {
  if (!entity) return null;

  return (
    <>
      <div className="glow-card p-4 mt-2">
        <h3 className="text-sm font-medium text-app-text mb-2">{entity.name}</h3>
        <span
          className="text-[10px] px-1.5 py-0.5 rounded inline-block mb-2"
          style={{
            background: `${ENTITY_COLORS[entity.type]}20`,
            color: ENTITY_COLORS[entity.type],
            border: `1px solid ${ENTITY_COLORS[entity.type]}40`,
          }}
        >
          {entity.type}
        </span>
        ...
        <div className="text-[10px] text-app-muted">Mentions: {entity.mentions_count}</div>
